...
final class FreeShippingPolicy
{
    ...
    public function apply_to_estimate(array $estimate, array $package = array(), ?float $threshold = null): array
    {
        $threshold = $threshold ?? $this->threshold();
        if (!$this->is_eligible($package, $threshold)) {
            return $estimate;
        }

        $carrier_price = isset($estimate['price']) ? $estimate['price'] : null;
        $carrier_label = (string) ($estimate['label'] ?? '');
        $days = isset($estimate['days']) ? (int) $estimate['days'] : null;
        $estimate['price'] = 0.0;
        $estimate['free_shipping'] = true;
        $estimate['free_shipping_threshold'] = $threshold;
        $estimate['subtotal_before_discounts'] = $this->subtotal_before_discounts($package);
        $estimate['carrier_price'] = $carrier_price;
        $estimate['carrier_label'] = $carrier_label;
        $estimate['label'] = __('Бесплатно', 'ranau-5post-for-woocommerce');
        if ($days !== null) {
            /* translators: %d: delivery time in days. */
            $estimate['label'] .= sprintf(__(', срок: %d дн.', 'ranau-5post-for-woocommerce'), $days);
        }
